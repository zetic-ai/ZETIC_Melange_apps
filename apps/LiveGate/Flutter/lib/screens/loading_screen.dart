...
/// Shown while Melange downloads + initializes BOTH models (PAD + FACE) and
/// warms them. Owns the [MelangeService] and [Enrollment] and hands them to the
/// main screen once ready.
...
class _LoadingScreenState extends State<LoadingScreen>
  ...
  Future<void> _load() async {
    ...
    try {
      await _enrollment.load();
      await _service.init(
        onProgress: (p) {
          if (!mounted) return;
          setState(() {
            _progress = p.clamp(0.0, 1.0);
            _status = 'Loading models… ${(_progress * 100).toStringAsFixed(0)}%';
          });
        },
      );
      if (!mounted) return;
        ...
        child: Padding(
          ...
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 40),
              Text(
                'Powered by ZETIC Melange · no cloud',
                style: TextStyle(
...
class _ErrorBlock extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(
      children: [
        const Icon(Icons.error_outline, color: GateColors.fail, size: 36),
