...
A fully on-device worker-safety PPE compliance demo for Flutter, powered by a YOLOv8s PPE detector (13 classes, 4 rendered) through the ZETIC Melange SDK. Product name: **SiteGuard**. Overlays worn-vs-violation color-coded boxes (Hardhat / NO-Hardhat / Safety Vest / NO-Safety Vest) with a latency + per-class count HUD. Built to tolerate a ~400 ms CPU-served artifact with ~5 ms NPU as the upside.

**Static-image pivot (2026-07-08):** the demo input source was converted from the live camera stream to a STILL IMAGE — pick a photo from the gallery (`image_picker`) or run one of the bundled `demo_validation` originals (`assets/samples/`). Because a real worksite/live camera is not available for the demo run, the same detector now runs on a chosen still image instead of a live feed. The pipeline is byte-for-byte identical (letterbox-640 + /255 + NCHW preprocess, channel-major `[1,17,8400]` decode, whitelist `{3,7,9,12}`, per-class thresholds, per-class NMS); only the INPUT SOURCE changed (`CameraImage` stream → decoded JPEG via the `image` package), and the overlay maps onto the static image rather than a live preview. The `camera` dependency + its runtime permission were removed from the main flow.
...
- [x] Static-image pivot: replaced the live-camera main flow with a still-image flow (gallery via `image_picker` + bundled `demo_validation` samples in `assets/samples/`, decoded to RGB via `image`), fed through `FrameData.fromBgraImage` into the UNCHANGED preprocess → decode → per-class-NMS pipeline; overlay maps onto the static image (sensorOrientation 0). Removed `camera` dependency + `camera_screen.dart` + the camera runtime permission from the main flow.
